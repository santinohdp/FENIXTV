...
app.get('/player_api.php', async (req, res) => {
  ...
  const serverInfo = {
    url: req.hostname, port: '80', https_port: '443',
  ...
  if (action === 'get_live_streams')    return res.json([{
    num: 1, name: 'Lista IPTV', stream_type: 'live', stream_id: 1,
    stream_icon: '', epg_channel_id: '', added: '', category_id: '1',
...
// live/:username/:password/:streamId — proxy de streams
app.get('/live/:username/:password/:streamId', async (req, res) => {
  const { username, password, streamId } = req.params;
  const user = await getUser(username, password);
  if (!user) return res.status(401).end();
  if (user.listType === 'xtream' && user.xtreamServer) {
    const ext = streamId.includes('.') ? '' : '.m3u8';
    return res.redirect(302, `${user.xtreamServer}/live/${user.xtreamUser}/${user.xtreamPass}/${streamId}${ext}`);
  }
  res.status(404).end();
});
